<template>
  <div
    class="min-h-screen bg-gray-50 dark:bg-[#0a0a0a] text-gray-900 dark:text-white pt-24 pb-16"
  >
    <div class="container mx-auto px-4">
      ...
      <div
        class="mb-8 rounded-2xl border border-gray-200 bg-white/80 p-4 backdrop-blur dark:border-white/10 dark:bg-[#111]/70"
      >
      ...
      >
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          />
          <input
            v-model="guestAuth.order_password"
            type="password"
            class="bg-gray-50 dark:bg-black/40 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-600 focus:outline-none focus:border-purple-500 transition-colors"
        ...
        >
          ...
          >
            <div>
              ...
              <div class="mt-2 text-xs text-gray-500">
                ...
                }}<span v-if="resultChannelType">
                  ({{ resultChannelType }})</span
                >
              </div>
            </div>
          ...
          <div class="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-4">
              ...
              <div
                v-else
                class="bg-gray-50 dark:bg-black/30 border border-gray-200 dark:border-white/10 rounded-2xl p-6"
              >
                <div class="text-sm text-gray-500 mb-3">
                  {{ t("payment.openPayLink") }}
                </div>
                <a
                  :href="paymentResult.pay_url"
                  target="_blank"
                ...
                <div
                  v-if="openedPayWindow"
                  class="mt-3 text-xs text-emerald-500"
                >
                  {{ t("payment.redirectOpened") }}
                </div>
                <div class="mt-3 flex flex-wrap items-center gap-2">
                  <button
                    @click="handleCopyPayLink"
                  ...
                  <span v-if="copied" class="text-xs text-emerald-500">{{
                    t("payment.copied")
                  }}</span>
                </div>
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 space-y-6">
          ...
          >
            ...
            >
              ...
              <div class="flex flex-col items-start md:items-end gap-2">
                ...
                <span class="text-xs text-gray-500 dark:text-gray-400"
                  >{{ t("payment.feeRateLabel") }}：{{ feeRateDisplay }}</span
                >
                <span class="text-xs text-gray-500 dark:text-gray-400"
            ...
            <div class="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
              <div
                class="bg-gray-50 dark:bg-black/30 border border-gray-200 dark:border-white/10 rounded-xl p-3"
              >
                <div class="text-xs text-gray-500">
                  {{ t("orderDetail.amountOriginal") }}
                </div>
                <div class="text-gray-900 dark:text-white font-mono mt-1">
                  {{ formatMoney(order.original_amount, order.currency) }}
                </div>
              </div>
              <div
                class="bg-gray-50 dark:bg-black/30 border border-gray-200 dark:border-white/10 rounded-xl p-3"
              ...
              >
                <div class="text-xs text-gray-500">
                  {{ t("orderDetail.promotionDiscountLabel") }}
                </div>
                <div class="text-gray-900 dark:text-white font-mono mt-1">
                  {{
                    formatMoney(order.promotion_discount_amount, order.currency)
                  }}
                </div>
              </div>
            </div>
            <div class="mt-3 text-sm text-gray-500 dark:text-gray-400">
              <span v-if="order.expires_at"
                >{{ t("payment.expiresAt") }}：{{
            ...
            <div v-if="pollingActive" class="mt-3 text-xs text-gray-500">
              {{ t("payment.pollingHint") }}
            </div>
          </div>
              ...
              >
                <div>
                  ...
                  <div class="text-xs text-gray-500 mt-1">
                    {{ t("orderDetail.quantityLabel") }}：{{ item.quantity }} ·
                    {{ t("orderDetail.itemFulfillmentLabel") }}：{{
                      fulfillmentTypeLabelText(item.fulfillment_type)
                    }}
                  </div>
                </div>
                <div class="text-xs text-gray-500">
                  {{ t("orderDetail.totalPriceLabel") }}：{{
            ...
            <div v-else>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <div class="text-xs text-gray-500 mt-1">
                    {{ providerTypeLabel(channel.provider_type) }} /
          ...
          >
            ...
            <div class="text-sm text-gray-600 dark:text-gray-400 space-y-2">
              <div>
                {{ t("payment.methodLabel") }}：{{
                  channelTypeLabel(paymentResult.channel_type)
                }}
                ({{ providerTypeLabel(paymentResult.provider_type) }})
              </div>
              <div>
                {{ t("payment.interactionLabel") }}：{{ interactionLabel }}
              </div>
            ...
            <div
              v-if="showQRCode"
              class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6 items-center"
            >
              <div
            ...
            <div
              v-if="showPayLink"
              class="mt-6 flex flex-col md:flex-row md:items-center gap-3"
            >
              <a
                :href="paymentResult.pay_url"
                target="_blank"
        ...
        <div
          class="h-fit rounded-2xl border border-gray-200 bg-white p-6 lg:sticky lg:top-24 dark:border-white/10 dark:bg-[#111]/80"
        >
          <h2 class="text-lg font-bold mb-4 text-gray-900 dark:text-white">
          ...
          >
            <div class="font-semibold">
              {{ t("payment.methodLabel") }}：{{ selectedChannel.name }}
            </div>
            <div class="mt-1">
              {{ providerTypeLabel(selectedChannel.provider_type) }} /
...
const isGuest = computed(
  () => route.query.guest === "1" || route.query.guest === "true",
);
...
const backLink = computed(() =>
  isGuest.value ? "/guest/orders" : "/me/orders",
);
const hasGuestAuth = computed(() =>
  Boolean(guestAuth.value.email && guestAuth.value.order_password),
);
const showGuestAuthForm = computed(
  () => isGuest.value && (!hasGuestAuth.value || guestAuthError.value),
);
...
const findChannelByID = (channelID: unknown) => {
  ...
  return (
    channels.value.find((item: any) => normalizeID(item?.id) === target) || null
  );
};

const selectedChannel = computed(() =>
  findChannelByID(selectedChannelId.value),
);

const cachedChannel = computed(() =>
  findChannelByID(cachedPayment.value?.channel_id),
);
...
const resultChannel = computed(() =>
  findChannelByID(paymentResult.value?.channel_id),
);

const resultChannelName = computed(() => {
  if (resultChannel.value?.name) return resultChannel.value.name;
  if (paymentResult.value?.channel_type)
    return channelTypeLabel(paymentResult.value.channel_type);
  return "-";
});
...
const showQRCode = computed(() => {
  const mode = String(
    paymentResult.value?.interaction_mode || "",
  ).toLowerCase();
...
const payLink = computed(() =>
  String(paymentResult.value?.pay_url || "").trim(),
);
...
const renderQRCodeImage = async () => {
  ...
  const isImageURL = /^https?:\/\/.+\.(png|jpe?g|gif|webp|svg)(\?.*)?$/i.test(
    qr,
  );
  if (isImageURL) {
    qrImageUrl.value = qr;
...
const showCountdown = computed(() =>
  Boolean(expiresAtMs.value && order.value?.status === "pending_payment"),
);
...
const feeAmountValue = computed(() => {
  if (
    paymentResult.value?.fee_amount !== undefined &&
    paymentResult.value?.fee_amount !== null &&
    paymentResult.value?.fee_amount !== ""
  ) {
...
const payableAmountDisplay = computed(() => {
  if (
    paymentResult.value?.amount !== undefined &&
    paymentResult.value?.amount !== null &&
    paymentResult.value?.amount !== ""
  ) {
    return formatMoney(
      String(paymentResult.value.amount),
...
const loadOrder = async (options?: { silent?: boolean }) => {
  ...
  try {
    if (isGuest.value) {
      ...
      const response = await guestOrderAPI.detailByOrderNo(
        orderNoQuery.value,
        {
      ...
      const response = await userOrderAPI.detailByOrderNo(orderNoQuery.value, {
        silentBusinessError: true,
      });
      order.value = response.data.data;
    }
...
const buildPayRouteQuery = () => {
  ...
  const resolvedOrderNo = String(
    order.value?.order_no || orderNoQuery.value || "",
  ).trim();
  if (resolvedOrderNo !== "") {
...
const capturePaypalIfNeeded = async () => {
  ...
  const providerType = String(
    paymentResult.value?.provider_type || "",
  ).toLowerCase();
  const channelType = String(
    paymentResult.value?.channel_type || "",
  ).toLowerCase();
  ...
  const payerId = String(
    route.query.payer_id || route.query.PayerID || "",
  ).trim();
  ...
  try {
    if (isGuest.value) {
      ...
      await guestOrderAPI.capturePayment(
        Number(paymentResult.value.payment_id),
        {
          email: guestAuth.value.email,
          order_password: guestAuth.value.order_password,
        },
...
const performPayment = async () => {
  ...
  if (
    cachedPayment.value &&
    selectedChannelId.value === cachedPayment.value.channel_id
  ) {
...
const redirectToOrderDetail = () => {
  ...
  const target = isGuest.value
    ? `/guest/orders/${resolvedOrderNo}`
    : `/orders/${resolvedOrderNo}`;
  window.setTimeout(() => {
    router.push(target);
  }, 600);
};
...
const fulfillmentTypeLabelText = (type: string) =>
  fulfillmentTypeLabel(t, type, "orderDetail");
...
watch(
  () => [
    ...
    route.query.pp_return,
    route.query.token,
    route.query.payer_id,
    route.query.PayerID,
    route.query.stripe_return,
    route.query.session_id,
    route.query.epay_return,
    order.value?.status,
  ],
  () => {
